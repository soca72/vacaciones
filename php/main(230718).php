...
$sql="BEGIN VAC_PRC_CONTARDIASRESTANTES(:idu ,:nombre,:apaterno,:amaterno,:departamento,:antiguedad,:fechaIngreso,:fechaCumple,:dias,:diasLaborables,:idud,:solPendientes,:direccionOut,:celularOut,:emailOut,:usuarioOut,:claveOut); END;";               
    ...
    error_log("Â¡La base de datos de Oracle no estÃ¡ disponible!", '/var/log/apache2/error.log');
 ...
 #spin.modal {
      ...
      ";
	    ...
	    echo "<td style='text-align: center; font-size: 103%' width='25%'>$busqueda";
            ...
            echo "<td style='text-align: center; font-size: 103%' width='25%' > Antiguedad  : <font id='fontAntiguedad' color='OrangeRed'>".$antiguedad." aÃ±os"."</font></td>";
...
";
   ...
   language: {
                        ...
                        return 'Ingresa minimo 4 carÃ¡cteres...';
                        }
                        },
  ajax: {
    url: 'livesearch2.php',
...
document.getElementById('cambiarSolicitudes').submit();
        ...
        ";
         $btnCancelar="<input type=button onclick=$('#busqueda').val('$idusuario').trigger('change'); style='width: 10em;  height: 3em; border-radius:10px;' value=Cancelar>";       
                ...
                <table class='table'>
                  <tr>
                      <td style='padding-left:16px;' align='center'><font size=$fontsize>Tipo de Permiso: ";
                  ...
                  $sql="BEGIN VAC_PRC_PERMISOSVACACIONES(:permisosvacaciones);END;";
                          ...
                          echo "<select id='selectPermiso' onchange='permiso(this.value);'><option value='AAA' selected>Seleccione...</option>";
                            while (($row = oci_fetch_array($cursor, OCI_BOTH)) != false) 
                                  {
                                    echo "<option onclick='permiso(\"".$row[0]."\")' value='$row[0]'>".$row[0]."</option>";
                                  }
                            echo "</select>";
                  echo "
         ...
         <td>
                    ...
                    <tr>
                        <td colspan='100%' style='text-align: center;'>                  
                          <input type=submit value='Solicitar' style=' width: 10em;  height: 3em; border-radius:10px; ' onclick='validarfechas()'>
                           $btnCancelar
                    </td>
                    <td></td>
                    <td></td>
...
if ($accion==1) {
echo "<script>
alert('SolicitÃ³ mÃ¡s dias de los que dispone');
</script>";
}
if ($accion==2) {
echo "<script>
...
if ($accion==4) {
  echo "<script>
      alert('No puede solicitar mÃ¡s vacaciones ya que todavia tiene unas pendiente de aprobacion');
        </script>";
}
if ($accion==5) {
  echo "<script>
      alert('Las fechas del permiso que solicitÃ³ coinciden con las de un permiso que ya tiene');
        </script>";
}
if ($accion==6) {
  echo "<script>
      alert('Ya tiene un cumpleaÃ±os asignado en ese aÃ±o');
        </script>";
}
if ($accion==7) {

      echo "<script>
                alert('OcurriÃ³ uno de los siguientes problemas: \\n\\n1)Solo puedes solicitar 2 dÃ­as o menos por aÃ±o\\n2)Tienes mÃ¡s de un aÃ±o de antigÃ¼edad, debes solicitarlo como dÃ­as de vacaciones');
            </script>";
}
if ($accion==8) {
