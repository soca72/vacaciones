...
//if($_SESSION["idusuario"]=="") header('Location:../index.php');
 ...
 $jefe=$_POST['Jefe'];
 $RH=$_POST['RH'];
 $iddepsol=$_POST['idsolicituddepto'];
 $modo=$_POST['modo'];
      ...
      if ($iddepartamento==29)
        {
            $accion='au';
            //echo "RH";
            $sql="BEGIN VAC_PRC_APROBARRECURSOSH(:idu,:idsol,:accion,:correo,:nombre,:comentarioSuperiores);END;";
            $stmt=oci_parse($conn,$sql);
            oci_bind_by_name($stmt,':idu',$idusuario,32);
            oci_bind_by_name($stmt,':idsol',$idsolicitud,32);
            oci_bind_by_name($stmt,':accion',$accion,32);
        ...
        else if($iddepartamento==30){
            $accion = 'ag';
            $sql = "BEGIN VAC_PRC_APROBARGERENTE(:idu,:idsol,:accion,:correo,:nombre,:comentarioSuperiores);END;";
            $stmt=oci_parse($conn,$sql);
            oci_bind_by_name($stmt,':idu',$idusuario,32);
            oci_bind_by_name($stmt,':idsol',$idsolicitud,32);
            oci_bind_by_name($stmt,':accion',$accion,32);
        ...
        {
            $accion='pa';
            $sql="BEGIN VAC_PRC_APROBARJEFEAREA(:idu,:idsol,:accion,:correo,:nombre,:comentarioSuperiores);END;";
            $stmt=oci_parse($conn,$sql);
            oci_bind_by_name($stmt,':idu',$idusuario,32);
            oci_bind_by_name($stmt,':idsol',$idsolicitud,32);
            oci_bind_by_name($stmt,':accion',$accion,32);
