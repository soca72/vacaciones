...
objetoestado.m6='APROBADAS POR GERENTE';
var objetodepartamento={};
var validaComentario = false;
var validaComentarioJefe = false;
var validaComentarioGerente = false;
var validaComentarioRh=false;
...
//************************************************ *****************************************************************************************************************************
          ...
          function aprobar(idsolicitud,idsolicituddepto,jefea,rh,gerente)
            {
            	/*
	        ...
	        /*   if(confirm('Seguro desea aprobar el permiso con id ' + idsolicitud +' ?'))
                {*/
                   ...
                   // $("#btncomentarioSuperiores").off('click');
                    $("#btncomentarioSuperiores").one("click",function(){
                        
                    var comentarioSuperiores = $("#comentarioSuperiores").val();

	            	$.get("apruebasolicitud.php", //url
	            			{idsolicitud:idsolicitud,idsolicituddepto:idsolicituddepto, Jefe:jefea, RH:rh , idusuario: idusuario ,accion:'a',iddepartamento:iddepartamento,comentarioSuperiores:comentarioSuperiores, Gerente:gerente }, //datos
	            			 function( data ) { //funcion si la peticion es exitosa
                                            //alert(data);
                                            mandafiltros();
					});
                                        
                                        $("#modalComentariosSuperiores textarea").val("");
                                        $('#modalComentariosSuperiores').modal('hide')
                  ...
                  if(confirm('Seguro desea cancelar el permiso con id ' + idsolicitud +' ?'))
		                {
			            	$.get( "cancelasolicitud.php", //url
			            				{
			            					idsolicitud:idsolicitud , idusuario: idusuario ,accion:'a',iddepartamento:iddepartamento,comentarioSuperiores:comentarioSuperiores
			            				}, //datos
			            			 function( data ) 
...
function agregafiltrosucursal(idsucursal, datos) {
    $('[class^="container-fluid divSuc_"]').show();

    let splitIds = datos.split(';').slice(0, -1); 
    
    if ($("#dep_" + idsucursal).is(':checked')) {
        if (!ocultarSuc.includes(idsucursal.toString())) {
            ocultarSuc.push(idsucursal.toString());
        }
        splitIds = splitIds.filter(id => !ocultarSuc.includes(id)); 

    }else{
        ocultarSuc = ocultarSuc.filter(id => id !== idsucursal.toString());

        splitIds = splitIds.filter(id => !ocultarSuc.includes(id)); 
    }

    if(ocultarSuc.length>0){
        splitIds.forEach(id => {
            $('#divSolicitudes div.divSuc_' + id).hide(); // Oculta las demás sucursales
        });
    }else{
        $('[class^="container-fluid divSuc_"]').show();
    }
...
function llenatabla(data, textStatus, 	xhr) {
							    ...
							    for (var i=0; i<lineas.length; i++) 
							    		{
                                                                            tr += "<div class='container-fluid divSuc_" + lineas[i].IDSUCURSAL + "'>";
											    ...
											    if ( typeof lineas[i].JEFEAPROBO=='undefined' && typeof lineas[i].RHAPROBO=='undefined' && typeof lineas[i].GERENTEAPROBO=='undefined' &&  lineas[i].TIPOACCION!="den") {
											        //clase = "<tr class='warning' id='tr"+lineas[i].IDSOLICITUD+"' onclick=mostrarComentario("+lineas[i].IDSOLICITUD+")>";
                                                                                                clase= "warning";
											    }
							    			...
							    			//******************************************************************
                                                                                ...
                                                                                if(lineas[i].TIPOACCION=='den'){
                                                                                    tr = tr + "<div class = 'row' title='Cancelado por: "+lineas[i].USUARIOMODIFICO+", con fecha: "+lineas[i].FECHAMODIFICO+"'>";
                                                                                }
							    			else{
                                                                                tr = tr + "<div class = 'row'>";
                                                                            }
                                                                              //  tr = tr + "<tr style='border-top:1px solid #000;' class='" + clase + "' id='tr"+lineas[i].IDSOLICITUD+"' onclick=mostrarComentario("+lineas[i].IDSOLICITUD+",validaComentario,validaComentarioJefe,validaComentarioRh,validaComentarioGerente)>";
                                                                                tr = tr + "<div id='checkboxa' class='col " + clase +" text-center'>" + (clase!='success' && clase!='danger'?"<input type='checkbox' class='"+(($("#idDep").val() != "29" && typeof lineas[i].JEFEAPROBO=='undefined') || ($("#idDep").val() == "29" && typeof lineas[i].RHAPROBO=='undefined')|| ($("#idDep").val() == "30" && typeof lineas[i].GERENTEAPROBO=='undefined')?"chkAprobar":"")+" " + (typeof lineas[i].RHAPROBO=='undefined'?"chkCancelar":"") + "' idSolicitud = '" + lineas[i].IDSOLICITUD + "' name='chkSolicitud' id='chk-" + lineas[i].IDSOLICITUD + "'>":"") + "</div>" +
								            "<div class='col "+ clase +" text-center'>" + lineas[i].IDSOLICITUD + "</div>" +
								            "<div class='col "+ clase +" text-center'><div class='esconder'>" + lineas[i].ORDENFECHAALTA + "</div>" + splitFechasAlta[0] + "<br>"+splitFechasAlta[1]+"</div>" +
								            "<div class='col "+ clase +" text-center'>" + lineas[i].SUCURSAL + "</div>" +
								            ...
								            "<div class='col "+ clase +" text-center'>" + (lineas[i].OPCIONPAGO=='CONSUELDO'?'Con sueldo':(lineas[i].OPCIONPAGO=='SINSUELDO'?'Sin sueldo':'Tiempo')) + "</div>";
												...
												if(lineas[i].FECHAAUTORIZOJEFEAREA || "")
								            		{
								            			...
								            			tr = tr + "<div class='col "+ clase +" text-center'><div class='esconder'>" + lineas[i].ORDENFECHAAUTORIZOJEFEAREA + "</div>" + splitFechasJefe[0] + "<br>"+splitFechasJefe[1]+ "</div>" ;
								            		}
								        		else
								        			{
								        				...
								        				tr = tr + "<div class='col "+ clase +" text-center'>" + "-" + "</div>" ;
                                                                                                ...
                                                                                                if(lineas[i].GERENTEAPROBO || "")
                                                                                                {
                                                                                                    tr = tr + "<div class='col "+ clase +" text-center'>" + lineas[i].GERENTEAPROBO + "</div>";
                                                                                                }
                                                                                                else
                                                                                                {
                                                                                                    tr = tr + "<div class='col "+ clase +" text-center'>" + "-" + "</div>";
                                                                                                }
                                                                                                if(lineas[i].FECHAAUTORIZOGERENTE || "")
                                                                                                {
                                                                                                    splitFechasGerente=lineas[i].FECHAAUTORIZOGERENTE.split(" ");
                                                                                                    tr = tr + "<div class='col "+ clase +" text-center'><div class='esconder'>" + lineas[i].ORDENFECHAAUTORIZOGERENTE + "</div>" + splitFechasGerente[0] + "<br>"+splitFechasGerente[1]+ "</div>";
                                                                                                }
                                                                                                else{
                                                                                                    tr = tr + "<div class='col "+ clase +" text-center'>" + "-" + "</div>" ;
                                                                                                }
								            		...
								            		{
								            			tr = tr + "<div class='col "+ clase +" text-center'><div class='esconder'>" + lineas[i].ORDENFECHAAUTORIZORECURSOSH + "</div>" + lineas[i].RHAPROBO + "</div>" ;
								            		}
								        		else
								        			{
								        			...
								        			if(lineas[i].FECHAAUTORIZORECURSOSH || "")
								            		{
								            			...
								            			tr = tr + "<div class='col "+ clase +" text-center'>" + splitFechasRH[0] + "<br>"+splitFechasRH[1]+ "</div>" ;
								            		}
								        		else
								        			{
				...
				tr = tr + "<div class='col "+ clase +" '><div class='dropdown'>"
       					+ "<button class='btn btn-info dropdown-toggle' type='button' data-toggle='dropdown'>Acciones"
                        + "<span class=''></span></button><ul class='dropdown-menu dropdown-menu-administrar'>";


                 if(my_in_array(4,permisosusuario))
                 {
        if ((typeof lineas[i].JEFEAPROBO=='undefined' || typeof lineas[i].RHAPROBO=='undefined' || typeof lineas[i].GERENTEAPROBO=='undefined') &&  lineas[i].TIPOACCION!="den" && lineas[i].TIPOACCION!="au") {
                
                
                if(($("#idDep").val() == "30" && typeof lineas[i].GERENTEAPROBO=='undefined' ) || ($("#idDep").val() == "29" && typeof lineas[i].RHAPROBO=='undefined') || ($("#idDep").val() != "30" && ($("#idDep").val() != "29" && typeof lineas[i].GERENTEAPROBO=='undefined' && typeof lineas[i].JEFEAPROBO=='undefined'  ))){    
//                tr = tr + " <li> \
...
//                   </li>";
                    ...
                    tr = tr + " <li> \
                       <a id='linkAprobar-" + lineas[i].IDSOLICITUD + "' href=javascript:aprobar("+lineas[i].IDSOLICITUD+","+lineas[i].IDDEPARTAMENTO+",'"+lineas[i].JEFEAPROBO+"','"+lineas[i].RHAPROBO+"','"+lineas[i].GERENTEAPROBO+"'); > \
                       <button type='button' class='btn btn-success' data-bs-toggle='modal' data-bs-target='#modalComentariosSuperiores' data-bs-color='success' data-bs-whatever1='Aprobar' data-bs-whatever='Aprobando ID: "+lineas[i].IDSOLICITUD+"' >\
                       <span class='glyphicon glyphicon-ok'></span>\
                       Aprobar &nbsp;\
                       </button>\
                       </a> \
                ...
                if(($("#idDep").val()=="29" && typeof lineas[i].RHAPROBO=='undefined') || (typeof lineas[i].RHAPROBO=='undefined' && typeof lineas[i].GERENTEAPROBO=='undefined' && typeof lineas[i].JEFEAPROBO=='undefined') || ( typeof lineas[i].JEFEAPROBO=='undefined' && typeof lineas[i].GERENTEAPROBO=='undefined' ) || ($("#idDep").val() != "29"  && typeof lineas[i].GERENTEAPROBO=='undefined' && typeof lineas[i].JEFEAPROBO=='undefined' && typeof lineas[i].FECHAAUTORIZOGERENTE =='undefined')|| ($("#idDep").val() == "30" && typeof lineas[i].GERENTEAPROBO=='undefined' || typeof lineas[i].JEFEAPROBO=='undefined' && typeof lineas[i].FECHAAUTORIZOGERENTE =='undefined') ){
                    tr = tr + "<li> \
                         <a href='javascript:denegar("+lineas[i].IDSOLICITUD+")'> \
                         <button type='button' class='btn btn-danger' data-bs-toggle='modal' data-bs-target='#modalComentariosSuperiores' data-bs-color='danger' data-bs-whatever1='Cancelar' data-bs-whatever='Cancelando ID: "+lineas[i].IDSOLICITUD+"'>\
                ...
                if(lineas[i].COMENTARIO.length<=1 && lineas[i].COMENTARIOJEFE.length<=1 && lineas[i].COMENTARIORH.length<=1 && lineas[i].COMENTARIOGERENTE.length<=1 ){
                    tr = tr + "<div class='col-12 " + clase + " comentario p-2'><b>Sin comentarios</b> </div>" ;
                }else{
                
                if(lineas[i].COMENTARIO.length>1){
                     tr = tr + "<div class='col-12 " + clase + " comentario  p-2'><b>Comentario del solicitante:</b> "  + (typeof lineas[i].COMENTARIOPREESTABLECIDO!='undefined'?lineas[i].COMENTARIOPREESTABLECIDO+" - ":"")+ lineas[i].COMENTARIO +"</div>" ;
                     //console.log("TIPO COM: "+typeof lineas[i].COMENTARIOPREESTABLECIDO + lineas[i].IDSOLICITUD );
                    validaComentario = true;
                }
                if(lineas[i].COMENTARIOJEFE.length>1 ){
                             tr = tr +"<div class='col-12 " + clase + " comentarioJefe  p-2'><b>Comentario de jefe área: </b> " + lineas[i].COMENTARIOJEFE + "</div>"
                    validaComentarioJefe = true;
                }
                
                if(lineas[i].COMENTARIOGERENTE.length>1 ){
                             tr = tr + "<div class='col-12 " + clase + " comentarioGerente  p-2'><b>Comentario gerente: </b> " + lineas[i].COMENTARIOGERENTE + "</div>"
                    validaComentarioGerente = true;
                }
                
                if(lineas[i].COMENTARIORH.length>1){
                             tr = tr + "<div class='col-12 " + clase + " comentarioRH  p-2'><b>Comentario de RH: </b> "+ lineas[i].COMENTARIORH+ "</div>"
                    validaComentarioRh = true;

                }
                
    }
    tr = tr + "</div></div>";
        //console.log(lineas[i].COMENTARIORH.length)
		...
		tr = tr + "\
		  <form id='formaprobar"+lineas[i].IDSOLICITUD+"' class='form-inline' method='POST' hidden>\
            ...
            $("#divSolicitudes >.row  > .container-fluid > .row").on("click", function(){
                ...
                if(!$(this).find(".comentarioGerente").is(":visible"))$(this).find(".comentarioGerente").slideDown(150);
                else $(this).find(".comentarioGerente").slideUp(150);
                if(!$(this).find(".comentarioRH").is(":visible"))$(this).find(".comentarioRH").slideDown(200);
                else $(this).find(".comentarioRH").slideUp(200);
            });
...
function mostrarComentario(idsolicitud,validaComentario,validaComentarioJefe,validaComentarioRh,validaComentarioGerente){
    console.log("Entro mostrarComentario"+idsolicitud);
...
//    }
    console.log(validaComentario);
    console.log(validaComentarioJefe);
    console.log(validaComentarioRh);
    console.log(validaComentarioGerente);
    
                if(validaComentario){
                 // var element1 = document.getElementById('comentario'+idsolicitud+'');
                var display =  $('#comentario'+idsolicitud+'').css("display");
                if(display!="none" ){
                $('#comentario'+idsolicitud+'').attr("style", "display:none");
                $('#tr'+idsolicitud).removeClass("sinBorde");
                }else{
                //element1.style.display = 'revert';
                $('#comentario'+idsolicitud+'').attr("style", "display:revert");
                $('#tr'+idsolicitud).addClass("sinBorde");}
        }
            
                if(validaComentarioJefe){
                 // var element1 = document.getElementById('comentario'+idsolicitud+'');
                var display =  $('#comentarioJefe'+idsolicitud+'').css("display");
                if(display!="none" ){
                $('#comentarioJefe'+idsolicitud+'').attr("style", "display:none");
               
                }else{
                //element1.style.display = 'revert';
                $('#comentarioJefe'+idsolicitud+'').attr("style", "display:revert");
                $('#tr'+idsolicitud).addClass("sinBorde");
            }}
        
        if(validaComentarioGerente){
                 // var element1 = document.getElementById('comentario'+idsolicitud+'');
                var display =  $('#comentarioGerente'+idsolicitud+'').css("display");
                if(display!="none" ){
                $('#comentarioGerente'+idsolicitud+'').attr("style", "display:none");
               
                }else{
                //element1.style.display = 'revert';
                $('#comentarioGerente'+idsolicitud+'').attr("style", "display:revert");
                $('#tr'+idsolicitud).addClass("sinBorde");
            }}
            
            
                if(validaComentarioRh){
                // var element1 = document.getElementById('comentario'+idsolicitud+'');
                var display =  $('#comentarioRH'+idsolicitud+'').css("display");
                if(display!="none" ){
                $('#comentarioRH'+idsolicitud+'').attr("style", "display:none");
               
                }else{
                //element1.style.display = 'revert';
                $('#comentarioRH'+idsolicitud+'').attr("style", "display:revert");
            }}
...
function abrirModal(titulo,boton){
    $("#modalComentariosSuperioresLabel").html(titulo);
  $("#modalComentariosSuperiores").modal("show");
}
