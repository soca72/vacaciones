...
{
	url="../php/filtrasolicitudes.php?idu="+idu+"&idFiltro="+idf+"&estados=5,7,2,3,1&orden=id";
	...
	console.log(JSON.parse(data));
							    ...
							    for (var i=0; i<lineas.length; i++) {
							    			    ...
							    			    if ( typeof lineas[i].JEFEAPROBO != 'undefined' && typeof lineas[i].RHAPROBO== 'undefined' ) {
                                                                                    ...
                                                                                    if ( (typeof lineas[i].JEFEAPROBO!= 'undefined' && typeof lineas[i].GERENTEAPROBO!='undefined' &&  typeof lineas[i].RHAPROBO!= 'undefined') || lineas[i].TIPOACCION=='au') {
                                                                                    ...
                                                                                    if ( typeof lineas[i].JEFEAPROBO=='undefined' && typeof lineas[i].GERENTEAPROBO=='undefined' &&  typeof lineas[i].RHAPROBO=='undefined' &&  lineas[i].TIPOACCION!="den") {
...
//                                                                                           console.log(typeof lineas[i].JEFEAPROBO);
//                                                                                           console.log(typeof lineas[i].GERENTEAPROBO);
//                                                                                           console.log(typeof lineas[i].RHAPROBO);
										...
										if(lineas[i].FECHAAUTORIZOJEFEAREA || "")
								            		{
								            			tr = tr + "<div class='col " + clase + " text-center'><div class='esconder'>" + lineas[i].ORDENFECHAAUTORIZOJEFEAREA + "</div>" + lineas[i].FECHAAUTORIZOJEFEAREA + "</div>" ;
                                                                                ...
                                                                                if(lineas[i].FECHAAUTORIZOGERENTE || "")
                                                                                        {
                                                                                                tr = tr + "<div class='col " + clase + " text-center'><div class='esconder'>" + lineas[i].ORDENFECHAAUTORIZOGERENTE + "</div>" + lineas[i].FECHAAUTORIZOGERENTE + "</div>";
                                                                                        }
                                                                                else    
                                                                                        {
                                                                                                tr = tr + "<div class='col " + clase + " text-center'>" + "-" + "</div>";
								        	...
								        	if(lineas[i].RHAPROBO || "")
                                                                                ...
                                                                                if(lineas[i].FECHAAUTORIZORECURSOSH || "")
								            		{
								            			tr = tr + "<div class='col " + clase + " text-center'><div class='esconder'>" + lineas[i].ORDENFECHAAUTORIZORECURSOSH + "</div>" + lineas[i].FECHAAUTORIZORECURSOSH + "</div>" ;
                                                                                ...
                                                                                if(lineas[i].COMENTARIO.length<=1 && lineas[i].COMENTARIOJEFE.length<=1 && lineas[i].COMENTARIOGERENTE.length<=1 && lineas[i].COMENTARIORH.length<=1 ){
                                                                                    tr += "<div class='col-12 " + clase + " comentario  p-2'><b>Sin comentarios</b> </div>" 
                                                                                }else{
                                                                                if(lineas[i].COMENTARIO.length>1){
                                                                                tr += "<div class='col-12 " + clase + " comentario  p-2'><b>Comentario del solicitante:</b> " + (typeof lineas[i].COMENTARIOPREESTABLECIDO!='undefined'?lineas[i].COMENTARIOPREESTABLECIDO+" - ":"")+ lineas[i].COMENTARIO + "</div>" 
                                                                                }if(lineas[i].COMENTARIOJEFE.length>1){
                                                                                tr += "<div class='col-12 " + clase + " comentarioJefe  p-2'><b>Comentario de jefe área: </b> " + lineas[i].COMENTARIOJEFE + "</div>"
                                                                                }if(lineas[i].COMENTARIOGERENTE.length>1){
                                                                                tr += "<div class='col-12 " + clase + " comentarioGerente  p-2'><b>Comentario gerente: </b> " + lineas[i].COMENTARIOGERENTE + "</div>"
                                                                                }if(lineas[i].COMENTARIORH.length>1){
                                                                                tr += "<div class='col-12 " + clase + " comentarioRH  p-2'><b>Comentario de RH: </b> "+ lineas[i].COMENTARIORH+ "</div>"
                                                                                }
                                                                            }
								        	tr = tr + "</div></div>";
